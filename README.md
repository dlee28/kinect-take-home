...
The reason for a score function vs a hard filter on the remaining fields is because the fields hold different values for each customer and hard filter may remove customers that are very similar in all fields but one.
For example, a customer in the age band 35-44 can still have similar value priorites to a customer in the age band 45-54.
All the fiends seem to compare to the current persona for matching but last_order_at within 30 days and return_rate > 0.3 are not. The reason for this we want to give some weight to customers that are recent buyers so the current persona can see more current purchases. And return_rate > 0.3 is a negative signal so that current persona does not follow buying behavior of a high returner.
...
### Chat system that knows reviews of the current produc from similar customers

We want to initiate a conversation wit the user about the current product by give a summary/highlight of the reviews of the current product written by similar customers.
...
There could be a case where a similar customer has not written a review. In that case, we pass the top 5 highly rated review to the llm. The top 5 highly rated review is also used for anonymous shoppers.

Trade offs:

- Convience of the user not having to scroll to the reviews section and being able to ask questions about the product or the reviews.
- Cost of tokens. By messaging the user first through the chat widget, we are using more tokens that if we were waiting for the user to ask a question or just simply displaying the reiews.
- No data on if providing the most relevant reviews to the user will lead to a more engaging conversation. This is data that would need to be collected through tracing or session data.
